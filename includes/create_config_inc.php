...
function create_config( $pParamHash ) {
	...
	// DSN-style hosts (pdo, and similarly-shaped types) embed the database name inline,
	// e.g. "firebird:dbname=localhost:rdmcloud;charset=utf8;" - write that back out as a
	// concatenation against $gBitDbName rather than a dead literal, so the two settings
	// can't silently drift apart later. Falls back to a plain literal for host strings
	// that don't contain the db name (a bare hostname, most non-DSN driver types).
	$dbNamePos = strlen( $gBitDbName ) ? strpos( $gBitDbHost, $gBitDbName ) : false;
	if( $dbNamePos !== false ) {
		$hostPrefix = substr( $gBitDbHost, 0, $dbNamePos );
		$hostSuffix = substr( $gBitDbHost, $dbNamePos + strlen( $gBitDbName ) );
		$gBitDbHostLine = "\"$hostPrefix\".\$gBitDbName.\"$hostSuffix\"";
	} else {
		$gBitDbHostLine = "\"$gBitDbHost\"";
	}
	...
	// A relative '../config/...' path resolves against PHP's *real* (symlink-followed)
	// working directory, not the requested site's URL path - on this desktop's
	// <site>/install -> ../_bw5/install -> bitweaver/install symlink chain, that silently
	// wrote into the bare dev checkout's own config/ instead of the actual site being
	// installed. Use DOCUMENT_ROOT (as kernel/includes/setup_inc.php's BIT_ROOT_PATH
	// does) so this lands in the right place regardless of symlinks.
...
// Database name
// This must come before \$gBitDbHost below - DSN-style hosts (pdo, etc.) reference
// \$gBitDbName inline, so it has to already be assigned by that point.
